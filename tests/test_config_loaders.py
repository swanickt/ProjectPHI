...
These tests cover ProjectPHI's small runtime config loaders:

- patient alias CSV manifests load into `patient_id -> aliases` mappings;
- custom regex JSON loads into the config shape accepted by the CSV workflow;
- protected clinical terms CSV loads into runtime protected-term config;
- blank rows and surrounding whitespace are handled consistently;
- invalid configs raise `ValueError`;
- validation errors avoid echoing raw aliases, terms, or regex patterns.

These tests do not run de-identification. They only validate config ingestion.
"""
...
def test_load_patient_alias_manifest_valid_synthetic_csv(tmp_path):
    """A valid alias manifest trims whitespace, skips blanks, and groups aliases."""
...
def test_load_patient_alias_manifest_rejects_missing_columns(tmp_path):
    """Missing alias-manifest columns fail without echoing alias values."""
    manifest = tmp_path / "aliases.csv"
...
def test_load_patient_alias_manifest_rejects_empty_values_with_row_number(tmp_path, contents):
    """Empty alias-manifest values fail with row context but not raw values."""
    manifest = tmp_path / "aliases.csv"
...
def test_load_custom_regexes_json_valid_synthetic_config(tmp_path):
    """A valid custom-regex JSON file is returned unchanged after validation."""
...
def test_load_custom_regexes_json_rejects_invalid_json(tmp_path):
    """Invalid JSON raises a sanitized `ValueError`."""
    config = tmp_path / "custom_regexes.json"
...
def test_load_custom_regexes_json_rejects_invalid_shape_without_raw_patterns(
    ...
    """Invalid custom-regex shapes fail without echoing raw regex patterns."""
    config = tmp_path / "custom_regexes.json"
    config.write_text(json.dumps(payload), encoding="utf-8")
...
def test_load_protected_clinical_terms_csv_valid_synthetic_config(tmp_path):
    """A valid protected-term CSV groups terms by rule and preserves term order."""
...
def test_load_protected_clinical_terms_csv_rejects_missing_columns(tmp_path):
    """Missing protected-term columns fail without echoing term values."""
    config = tmp_path / "protected_terms.csv"
...
def test_load_protected_clinical_terms_csv_rejects_empty_values_safely(
    ...
    """Empty protected-term fields fail with row context but not raw term values."""
    config = tmp_path / "protected_terms.csv"
